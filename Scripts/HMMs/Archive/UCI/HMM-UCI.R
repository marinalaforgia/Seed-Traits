...
# Prep Data ####
uci <- read.csv("Data/Long-Term-Datasets/UCI/VegQuadDataWith2022.csv")
# meta <- read.csv("Data/Long-Term-Datasets/UCI/TransectsThrough22Meta_Nov1.csv")
# meta <- filter(meta, Passive_Resoration == "Y")

meta <- read.csv("Data/Long-Term-Datasets/UCI/OC_TX_To2022_ReadyToAnalyze_Nov1.csv")
meta <- filter(meta, Passive_Resoration == "Y")

uci <- filter(uci, TN_ID %in% meta$TN_ID)
# sp <- unique(uci$Full_Name) #515 LOOOOL
# sp <- unique(uci$Code) #526 codes?! 
# 
# counts <- count(uci, Code)
# counts <- filter(counts, n > 1)
# sp <- filter(, Code %in% counts)
# rm <- c("B", "L", "TH", "C", "R", "F", "M", "C", "W") # rm unknowns, perennials

uci <- pivot_longer(uci, cols = Brassica.geniculata:Sum.NS, names_to = "Species.Name", values_to = "Cover")
