...
plot.size <- read.csv("plot-sizes.csv")
...
trait <- read.csv("Species_list_2016_2.csv")
...
SD.df <- expand.grid(Species_Name = names(species.list), boot = 1:100, p0 = NA, g = NA, c = NA, s = NA, r = NA, iter = NA, n.plots = NA, min.years = NA, max.years = NA, mean.years = NA) # empty df to fill with rates
...
boot.df <- foreach(j = names(species.list), .combine = rbind) %dopar% { # for each species
  for(k in 1:100) {
    ...
    SD.df[SD.df$Species_Name == j & SD.df$boot == k,]$p0 <- EMresult$param$p0
    SD.df[SD.df$Species_Name == j & SD.df$boot == k,]$g <- EMresult$param$g
    SD.df[SD.df$Species_Name == j & SD.df$boot == k,]$c <- EMresult$param$c
    SD.df[SD.df$Species_Name == j & SD.df$boot == k,]$s <- EMresult$param$s
    SD.df[SD.df$Species_Name == j & SD.df$boot == k,]$r <- EMresult$param$r
    SD.df[SD.df$Species_Name == j & SD.df$boot == k,]$iter <- which.max(EMresult$lllist)
...
saveRDS(boot.df, "SD-species-boot.RDS")
