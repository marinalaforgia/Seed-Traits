...
cp <- readRDS("CP-sim-species-boot.RDS")
cp <- as.data.frame(cp)

cp <- cp %>% 
  dplyr::mutate(across(p0.boot:iter.boot.sd, ~as.numeric(.x)))

colnames(cp)[1] <- "Code"

cp <- filter(cp, s.boot.sd < 0.1, c.boot.sd < 0.1, perc.con > 0.5)
...
sim.df <- foreach(j = rownames(cp), .combine = rbind) %dopar% { # for each species
  
  sim <- list()
  
  for(k in 1:100){
    sim[[k]] <- simulation(param[j,], N = patch, tf = 14)
...
saveRDS(sim.df, "CP-sim-species-validation-30.RDS")
