...
port <- readRDS("port-species-boot.RDS")
port <- as.data.frame(port)

port <- port %>% 
  dplyr::mutate(across(p0.boot:perc.con, ~as.numeric(.x)))

port <- filter(port, s.boot.sd < 0.1, c.boot.sd < 0.1, perc.con > 0.5)
...
sim.df <- foreach(j = rownames(param), .combine = rbind) %dopar% { # for each species
  ...
  for(k in 1:100){
    sim[[k]] <- simulation(param[j,], N = patch, tf = 22)
...
saveRDS(sim.df, "port-sim-species-validation-30.RDS")
